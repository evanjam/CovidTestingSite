...
//inserts username and password data into user_login table
	...
	if (isset($_POST['register'])) { //checks if the register button was pressed on index.php
		...
		$select_user = "SELECT * FROM user_login WHERE username = '$username'"; //prepares query statement to check if username already exists
		$result = $connect->query($select_user); //runs $sql_statement1 query and stores the result in $result
